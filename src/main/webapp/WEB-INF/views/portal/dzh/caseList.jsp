    ...
    <article>
		...
		<section class="met_article_head">
			...
			<div class="met_position">
				...
				<a href="${portalPath}/company/caseList" >客户案例</a> > 
...
var pageSize=10;
  $(function () {
	...
	function intentDetail(id){
		window.open("${portalPath}/company/caseDetail?id="+id);
	}
	...
	function getCaseListData(){
		 $.post("${portalPath}/cms/cmsEducation/getList", {
			...
			if (data.status === "success") {
				...
				$.each(data.data,function(index,item){
					htmls+='<li onclick=intentDetail("'+item.id+'")><dl><dt><a href="javascript:void(0);">';
					htmls+='<img src="'+item.image+'" width ="190" height="150"/></a></dt>';
					htmls+='<dd><div class="met_listbox">';
					htmls+='<h2><a href="javascript:void(0);">'+item.title+'</a></h2>';
					
					if(item.description!=''&&item.description.length>150){
						htmls+='<p>'+item.description.substr(0,150)+'...</p>';
					}else{
						htmls+='<p>'+item.description+'</p>';
					}
					
					htmls+='</div></dd></dl><div class="met_clear"></div></li>';
