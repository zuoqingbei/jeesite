...
<body>
	...
	<form id="listForm" method="post">
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			<tr><th>栏目名称</th><!-- <th>归属机构</th><th>栏目模型</th> --><th style="text-align:center;">排序</th>
			<th title="是否在导航中显示该栏目">导航菜单</th>
			<!-- <th title="是否在分类页中显示该栏目的文章列表">栏目列表</th><th>展现方式</th> --><th>操作</th></tr>
			<c:forEach items="${list}" var="tpl">
				...
				<%-- 	<td>${fns:getDictLabel(tpl.inList, 'show_hide', '隐藏')}</td>
					...
					<td>
						<a href="${pageContext.request.contextPath}${fns:getPortalPath()}/${tpl.href}" target="_blank">访问</a>
						<shiro:hasPermission name="cms:category:edit">
