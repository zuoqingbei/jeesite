...
public class sqlYuJu {
	...
	public String keHuTongJi() {
		String sql = "SELECT"
				...
				+ " HDTJ.JYSMC,"
				+ " HRJ.JYSINFO,"
				+ " (SELECT COUNT(DISTINCT(HTYC.KHH)) FROM HUB_TQS_YQ_CPHY_gf HTYC WHERE HTYC.JYS = HDTJ.JYS) RZRKHS,"
	...
	// 所有产品
	public String suoyouchanpin() {
		String sql = "SELECT * FROM HUB_QY_CPFL order by cplb";
		return sql;
	}
	...
	// 年化利率
	public String nianhualilv() {
		String sql = "SELECT" + " HQC1.CPLB,"
	...
	// 产品统计
	public String chanpintongji() {
		String sql = "SELECT"
				+ " HQC1.CPLB,"
				+ " COALESCE(ROUND((SELECT AVG(IYR.FVALUE) FROM INSIGHT_YIELD_RATE IYR"
				+ " LEFT JOIN HUB_QY_CPLB HQC2"
				+ " ON HQC2.CPDM = IYR.CPBM"
				+ " WHERE HQC1.CPLB = HQC2.CPLB AND TO_DATE(VDAY,'YYYYMMDD') <= CURRENT_DATE"
				+ " AND TO_DATE(VDAY,'YYYYMMDD') > CURRENT_DATE - CAST(DATE_PART('DAY',CURRENT_DATE) - 1||' DAY' AS INTERVAL)),4),0) PJLL,"
				+
				...
				+ " ON HQC2.CPDM = IYR.CPBM"
				+ " WHERE HQC1.CPLB = HQC2.CPLB AND TO_DATE(VDAY,'YYYYMMDD') <= CURRENT_DATE"
				+ " AND TO_DATE(VDAY,'YYYYMMDD') > CURRENT_DATE - CAST(DATE_PART('DAY',CURRENT_DATE) - 1||' DAY' AS INTERVAL)) - "
				...
				+ " AND TO_DATE(VDAY,'YYYYMMDD') <= CURRENT_DATE - CAST(DATE_PART('DAY',CURRENT_DATE)||' DAY' AS INTERVAL))) / "
				+ " (SELECT AVG(IYR.FVALUE) FROM INSIGHT_YIELD_RATE IYR"
				+ " LEFT JOIN HUB_QY_CPLB HQC2"
				+ " ON HQC2.CPDM = IYR.CPBM"
				+ " WHERE HQC1.CPLB = HQC2.CPLB AND TO_DATE(VDAY,'YYYYMMDD') >= CURRENT_DATE - INTERVAL'1 MONTH' - CAST(DATE_PART('DAY',CURRENT_DATE) - 1||' DAY' AS INTERVAL)"
